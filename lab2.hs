...
fromDecimal toBase snumber = helper toBase (wordToInt(snumber)) [] where
                             ...
                             helper toBase number acc = helper toBase (div number toBase) ((chr ((if (mod number toBase) < 10 then (mod number toBase+48) else (if ((mod number toBase) > 9 && (mod number toBase) < 36) then (mod number toBase+87) else (if ((mod number toBase) > 35 && (mod number toBase) < 62) then (mod number toBase+29) else error "digit is not ok")))):acc))
                             wordToInt [] = 0
                             wordToInt (x:word) = (digitToInt'(x)*10^length(word) + wordToInt word)
                             digitToInt' a = (if ((ord a > 47) && (ord a < 58) && (ord a - 48 < 10)) then (ord a - 48) else error "digit is not ok")
